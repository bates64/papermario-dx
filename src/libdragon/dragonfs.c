...
void dma_read(void * ram_address, unsigned long pi_address, unsigned long len) {
    debug_printf("dma_read: %p -> %p", pi_address, ram_address);
    // FIXME: crashes
    dma_copy((u8*)(pi_address), (u8*)(pi_address + len), ram_address);
    debug_printf("read complete\n");
}

void data_cache_hit_writeback_invalidate(volatile void * addr, unsigned long length) {
    // osWritebackDCache((void*)addr, length);
    // osInvalDCache((void*)addr, length);
}
