...
enum ProfilerTime {
    PROFILER_TIME_FPS = 0,
    PROFILER_TIME_CONTROLLERS = 1,
    PROFILER_TIME_WORKERS = 2,
    PROFILER_TIME_TRIGGERS = 3,
    PROFILER_TIME_EVT = 4,
    PROFILER_TIME_MESSAGES = 5,
    PROFILER_TIME_HUD_ELEMENTS = 6,
    PROFILER_TIME_STEP_GAME_MODE = 7,
    PROFILER_TIME_ENTITIES = 8,
...
#else
    PROFILER_TIME_AUDIO = 23,
#endif
    PROFILER_TIME_TOTAL = 24,
    PROFILER_TIME_RSP_GFX = 25,
    PROFILER_TIME_RSP_AUDIO = 26,
    PROFILER_TIME_TMEM = 27,
    PROFILER_TIME_PIPE = 28,
    PROFILER_TIME_CMD = 29,

    // state_world
    PROFILE_TIME_WORLD_ENCOUNTERS = 30,
    PROFILE_TIME_WORLD_NPCS = 31,
    PROFILE_TIME_WORLD_PLAYER = 32,
    PROFILE_TIME_WORLD_ITEM_ENTITIES = 33,
    PROFILE_TIME_WORLD_EFFECTS = 34,
    PROFILE_TIME_WORLD_CAMERAS = 35,

    PROFILER_TIME_COUNT = 36 // Must be last!
};
...
static ALWAYS_INLINE void profiler_gfx_subset_complete_and_switch_func(
    enum ProfilerTime complete1, enum ProfilerTime complete2, enum ProfilerTime start
) {
    u32 time;
    OS_GET_COUNT_INLINE(time);
...
static ALWAYS_INLINE void profiler_audio_subset_complete_and_switch_func(
    enum ProfilerTime complete1, enum ProfilerTime complete2, enum ProfilerTime start
) {
    u32 time;
    OS_GET_COUNT_INLINE(time);
...
// These two are unused by the default audio profiler; left in for cases of manual profiling of smaller functions as
// needed
