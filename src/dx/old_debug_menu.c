...
void dx_debug_menu_draw_contents(void* arg0, s32 baseX, s32 baseY, s32 width, s32 height, s32 opacity, s32 darkening) {
    ...
    while (item && item->name != NULL) {
        ...
        dx_old_string_to_msg(msgbuf, item->name);
...
void dx_render_debug_menu(void) {
    ...
    if (!dx_debug_menu_isVisible) {
        if (gGameStatus.pressedButtons[0] & (BUTTON_D_RIGHT | BUTTON_D_LEFT)) {
    ...
    dx_old_string_to_msg(msgbuf, dx_debugMenu.title);
...
u8* dx_old_string_to_msg(u8* msg, const u8* str) {
    while (*str) {
