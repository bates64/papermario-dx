...
API_CALLABLE(N(PlayLavaBubbleFlightSound)) {
    ...
    if (isInitialCall) {
        ...
        sfx_play_sound_at_position(
            SOUND_EMBER_FLY, SOUND_SPACE_DEFAULT, actor->curPos.x, actor->curPos.y, actor->curPos.z
        );
    }
    ...
    sfx_play_sound_at_position(
        SOUND_EMBER_FLY | SOUND_ID_TRIGGER_CHANGE_SOUND, SOUND_SPACE_DEFAULT, actor->curPos.x, actor->curPos.y,
        actor->curPos.z
    );
