...
typedef struct HudElement {
    ...
    /* 0x20 */ Vec2f deltaSize;
    /* 0x28 */ Vec2f dynamicSize;
...
// collects, z-sorts, and draws all hud elements belonging to the 'backUI' pass, which renders
// beneath windows and message boxes. does not render elements with a transformation being applied.
void render_hud_elements_backUI(void);

// second pass for rendering hud elements
// collects, z-sorts, and draws all hud elements belonging to the 'frontUI' pass, which renders
// on top of windows and message boxes. does not render elements with a transformation being applied.
void render_hud_elements_frontUI(void);

// additional pass for registered hud elements associated with CAM_HUD or CAM_BATTLE
void render_transformed_hud_elements(void);

// immediately draw the first complex (transformed) hud element using CAM_HUD, including all initial setup
// use an ID of -1 to only perform the setup
void hud_element_draw_complex_hud_first(s32 hid);

// immediately draw a subsequent complex (transformed) hud element using CAM_HUD, skipping any initial setup
void hud_element_draw_complex_hud_next(s32 hid);

// immediately draw the first complex (transformed) hud element using CAM_BATTLE, including all initial setup
// use an ID of -1 to only perform the setup
void hud_element_draw_complex_battle_first(s32 hid);

// immediately draw a subsequent complex (transformed) hud element using CAM_BATTLE, skipping any initial setup
