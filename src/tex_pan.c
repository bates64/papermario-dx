...
API_CALLABLE(UpdateTexturePanSmooth) {
...
API_CALLABLE(UpdateTexturePanStepped) {
    if (isInitialCall) {
...
EvtScript EVS_UpdateTexturePan = {
    SetGroup(EVT_GROUP_NEVER_PAUSE)
    IfEq(LVar5, 1)
        IfEq(LVar6, 1)
            IfEq(LVar7, 1)
                IfEq(LVar8, 1)
                    Call(UpdateTexturePanSmooth)
                    Return
                EndIf
            EndIf
        EndIf
    EndIf
    Call(UpdateTexturePanStepped)
    Return
    End
};
