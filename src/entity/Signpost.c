...
void entity_Signpost_idle(Entity* entity) {
    ...
    f32 val = fabsf(
        clamp_angle(180.0f - entity->rot.y)
    ...
    if (!(playerStatus->animFlags & PA_FLAG_USING_WATT)
        && !(entity->collisionFlags & ENTITY_COLLISION_PLAYER_TOUCH_FLOOR) && (val <= 40.0f || val >= 320.0f))
    {
...
EntityModelScript Entity_Signpost_RenderScript =
    STANDARD_ENTITY_MODEL_SCRIPT(Entity_Signpost_Render, RENDER_MODE_SURFACE_OPA);
