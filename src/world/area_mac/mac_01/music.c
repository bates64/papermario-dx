...
EvtScript N(EVS_SetupMusic) = {
    Switch(GB_StoryProgress)
        CaseEq(STORY_INTRO)
            Call(SetMusic, 0, SONG_PEACHS_CASTLE_PARTY, 1, 8)
        CaseEq(STORY_EPILOGUE)
            Call(SetMusic, 0, SONG_PEACHS_CASTLE_PARTY, 0, 8)
...
EvtScript N(EVS_PlaySpellcastSong) = {
    Call(SetMusic, 0, SONG_MERLEE_SPELL, 1, 8)
    Return
    End
};
...
EvtScript N(EVS_PlayFlowerGateSong) = {
    Call(SetMusic, 0, SONG_FLOWER_GATE_APPEARS, 0, 8)
    Return
    End
};
