...
EvtScript N(EVS_NpcIdle_Door) = {
    ...
    Thread
        Call(SetMusic, 0, SONG_FINAL_BOWSER_BATTLE, 1, 8)
        Wait(150)
        ...
        IfNe(MF_DoneIntroMessage, FALSE)
            Goto(5)
        EndIf
        Call(SetMusic, 0, SONG_BOWSERS_CASTLE, 0, 8)
        Wait(30)
        IfNe(MF_DoneIntroMessage, FALSE)
            Goto(5)
        EndIf
    ...
    Wait(2)
    Call(SetMusic, 0, SONG_KOOPA_BROS_THEME, 2, 8)
    Set(LVar3, 65)
    Set(LVar4, 0)
    Set(LVar5, 150)
    ...
    Call(SetMusic, 0, SONG_JR_TROOPA_THEME, 1, 8)
