    ...
    Switch(LVar0)
        CaseEq(flo_00_ENTRY_8)
            IfNe(AF_FLO_RidingBeanstalk, FALSE)
                Call(SetMusic, 0, SONG_MAGIC_BEANSTALK, 1, 8)
            Else
                Call(SetMusic, 0, SONG_FLOWER_FIELDS_SUNNY, 0, 8)
            EndIf
        CaseEq(flo_00_ENTRY_9)
            Call(SetMusic, 0, SONG_SUNSHINE_RETURNS, 0, 8)
        CaseEq(flo_00_ENTRY_A)
            Call(SetMusic, 0, SONG_STAR_SPIRIT_THEME, 1, 8)
            Thread
                Loop(0)
                    Wait(1)
        ...
        CaseDefault
            Switch(GB_StoryProgress)
                CaseLt(STORY_CH6_DESTROYED_PUFF_PUFF_MACHINE)
                    Call(SetMusic, 0, SONG_FLOWER_FIELDS_CLOUDY, 0, 8)
                CaseDefault
                    Call(SetMusic, 0, SONG_FLOWER_FIELDS_SUNNY, 0, 8)
            EndSwitch
    EndSwitch
