...
EvtScript N(EVS_PlayIntroStoryMusic) = {
    Wait(40)
    Call(SetMusic, 0, SONG_INTRO_STORY, 0, 8)
    Wait(2030)
    Call(SetMusic, 0, SONG_INTRO_STORY, 1, 8)
    Return
    End
};
...
EvtScript N(EVS_SetupMusic) = {
    ...
    Switch(LVar0)
        CaseEq(hos_05_ENTRY_1)
            Call(SetMusic, 0, SONG_STARSHIP_THEME, 1, 8)
        ...
        CaseDefault
            IfEq(AF_HOS_B5, FALSE)
                Call(SetMusic, 0, SONG_STAR_SANCTUARY, 0, 8)
            EndIf
            Call(PlaySound, SOUND_LOOP_STAR_SANCTUARY_INSIDE)
    EndSwitch
    Return
