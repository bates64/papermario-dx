...
typedef struct EndChapter {
    /* 0x00 */ Vec3f pos;
    /* 0x0C */ f32 yaw;
    /* 0x10 */ f32 angularVelocity;
    /* 0x14 */ f32 startYaw;
    /* 0x18 */ b16 showNarrationWindow;
    /* 0x1A */ s16 stopAccelerating;
    /* 0x1C */ s16 chapter;
    /* 0x1E */ s16 screenWhiteness;
    /* 0x20 */ EffectInstance* spiritCardEffect;
...
INCLUDE_IMG("world/area/kmr/kmr_23/window_ul.png", kmr_23_window_ul_img);
INCLUDE_IMG("world/area/kmr/kmr_23/window_ur.png", kmr_23_window_ur_img);
...
API_CALLABLE(N(CreateEndChapterData)) {
    ...
    npc->rot.y = data->yaw;
    backFacing = false;
    if (!evt_get_variable(script, MF_SpiritReleased)) {
...
enum NarrationWindowStates {
    NARRATION_WINDOW_IDLE       = 0,
    NARRATION_WINDOW_OPENING    = 1,
    NARRATION_WINDOW_OPEN       = 2,
    NARRATION_WINDOW_CLOSING    = 3,
    NARRATION_WINDOW_CLOSED     = 4,
};
