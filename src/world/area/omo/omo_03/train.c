...
EvtScript N(EVS_80246108) = {
    ...
    Else
        ...
        IfEq(AB_OMO_6, 0)
            ...
            Set(MV_TrainPath, Ref(N(D_802460A0_DAB2E0)))
        ...
        Else
            Set(MV_TrainRideState, 0)
            Set(MV_TrainPath, Ref(N(D_802460D0_DAB310)))
            Set(MV_TrainUnk_02, 1)
            Set(MF_TrainRideActive, true)
            Wait(1)
            Exec(N(EVS_TrainUnk_H))
            Label(30)
            IfLt(MV_TrainPosX, 350)
                Wait(1)
                Goto(30)
            EndIf
...
EvtScript N(D_80246538_DAB778) = {
    ...
    Set(MV_TrainRideState, 0)
    Set(MV_TrainPath, Ref(N(D_80246510_DAB750)))
    Set(MV_TrainUnk_02, 1)
    Set(MF_TrainRideActive, true)
    Label(10)
    IfLt(MV_TrainPosX, 350)
...
EvtScript N(D_802466F8_DAB938) = {
    ...
    ExecWait(N(EVS_TrainUnk_D))
    Set(MV_TrainRideState, 0)
    Set(MV_TrainPath, Ref(N(D_802466C0_DAB900)))
    Set(MV_TrainUnk_02, 1)
    Set(MF_TrainRideActive, true)
    Label(10)
    IfGt(MV_TrainPosX, -350)
        Wait(1)
        Goto(10)
    EndIf
...
EvtScript N(D_802468F8_DABB38) = {
    ...
    IfEq(AB_OMO_6, 0)
        Set(MV_TrainRideState, 0)
        Set(MV_TrainPath, Ref(N(D_80246880_DABAC0)))
    ...
    Else
        ...
        Set(MV_TrainRideState, 0)
        Set(MV_TrainPath, Ref(N(D_802468B0_DABAF0)))
...
EvtScript N(D_80246C80_DABEC0) = {
    ...
    Call(ModifyColliderFlags, MODIFY_COLLIDER_FLAGS_SET_BITS, COLLIDER_o941, COLLIDER_FLAGS_UPPER_MASK)
    IfEq(GF_OMO03_BlueSwitchActivated, false)
        Set(MV_TrainPath, Ref(N(D_80246C18_DABE58)))
    Else
        IfEq(AB_OMO_6, 0)
            Set(MV_TrainPath, Ref(N(D_80246BE0_DABE20)))
        Else
            Goto(15)
        EndIf
    EndIf
    ...
    Exec(N(EVS_TrainUnk_A))
    Set(MV_TrainRideState, 0)
    Set(MV_TrainPath, Ref(N(D_80246C40_DABE80)))
    Set(MV_TrainUnk_02, 0)
    Exec(N(EVS_Scene_RideTrain))
    Set(MF_TrainRideActive, true)
    Wait(1)
...
EvtScript N(EVS_8024746C) = {
    ...
    IfEq(LVar1, 1)
        Set(MV_TrainRideState, 0)
        Set(MV_TrainPath, Ref(N(D_80247454_DAC694)))
        Set(MV_TrainUnk_02, 0)
        Exec(N(EVS_Scene_RideTrain))
        Set(MF_TrainRideActive, true)
    Else
