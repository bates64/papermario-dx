...
EvtScript N(EVS_UnlockUpperLeftDoors) = {
    ...
    Call(GetEntityPosition, MV_EntityID_PadlockLower, LVar0, LVar1, LVar2)
    Call(PlaySoundAt, SOUND_USE_KEY, SOUND_SPACE_DEFAULT, LVar0, LVar1, LVar2)
    Call(SetEntityUsed, MV_EntityID_PadlockUpper)
    ResumeGroup(EVT_GROUP_FLAG_INTERACT)
    Unbind
    Return
    End
...
EvtScript N(EVS_UnlockLowerRightDoors) = {
    ...
    Call(GetEntityPosition, MV_EntityID_PadlockLower, LVar0, LVar1, LVar2)
    Call(PlaySoundAt, SOUND_USE_KEY, SOUND_SPACE_DEFAULT, LVar0, LVar1, LVar2)
    Call(SetEntityUsed, MV_EntityID_PadlockLower)
    ResumeGroup(EVT_GROUP_FLAG_INTERACT)
    Unbind
    Return
    End
...
EvtScript N(EVS_DropSwitch) = {
    ...
    Loop(0)
        Call(UpdateLerp)
        Call(SetEntityPosition, MV_EntityID_Switch, -180, LVar0, -35)
        IfEq(LVar1, 0)
            BreakLoop
        EndIf
        Wait(1)
...
EvtScript N(EVS_MakeEntities) = {
    ...
    IfLe(GB_StoryProgress, STORY_CH1_DEFEATED_BASEMENT_GUARD)
        ...
        IfEq(GB_StoryProgress, STORY_CH1_DEFEATED_BASEMENT_GUARD)
            Call(SetEntityPosition, MV_EntityID_Switch, -180, 0, -35)
        EndIf
    EndIf
    Return
    End
