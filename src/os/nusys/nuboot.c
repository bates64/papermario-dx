...
void nuBoot(void) {
    osInitialize();
    osCreateThread(&IdleThread, NU_IDLE_THREAD_ID, boot_idle, nullptr, &IdleStack[NU_SC_STACK_SIZE / sizeof(u64)], 10);
...
void boot_idle(void* data) {
    ...
    osCreateThread(&MainThread, NU_MAIN_THREAD_ID, boot_main, nullptr, &nuMainStack[NU_SC_STACK_SIZE / sizeof(u64)], NU_MAIN_THREAD_PRI);
    osStartThread(&MainThread);
    osSetThreadPri(&IdleThread, NU_IDLE_THREAD_PRI);

    while (1) {
        if (nuIdleFunc != nullptr) {
            nuIdleFunc();
        }
    }
}
