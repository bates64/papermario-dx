...
typedef struct action_command_overlay {
    void* handle;
    API_CALLABLE((*init));
    API_CALLABLE((*start));
    void (*free)(void); // TODO: replace with [[destructor]]
    void (*update)(void);
    void (*draw)(void);
} action_command_overlay;

// TODO: migrate overlay-specific state to the overlays themselves
typedef struct ActionCommandStatus {
    action_command_overlay overlay;
    /* 0x00 */ s32 workerID;
    ...
    /* 0x4A */ s16 actionCommandID; // TODO(decouple): remove
    /* 0x4C */ s16 state;
