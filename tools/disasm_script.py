...
class ScriptDisassembler:
    ...
    def disassemble_command(self, opcode, argc, argv):
        if opcode == 0x01:
            ...
            self.write_line(f"EVT_CONTINUE_LOOP")
        elif opcode == 0x09:
            ...
            self.write_line(f"EVT_IF_RANGE({self.var(argv[0])}, {self.var(argv[1])}, {self.var(argv[2])})")
            self.indent += 1
        elif opcode == 0x12:
            self.write_line(f"EVT_IF_NOT_RANGE({self.var(argv[0])}, {self.var(argv[1])}, {self.var(argv[2])})")
            self.indent += 1
        elif opcode == 0x13:
            ...
            self.write_line(f"EVT_NEG({', '.join(args)})")
        elif opcode == 0x34:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_NEGF({', '.join(args)})")
        elif opcode == 0x35:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_ABS({', '.join(args)})")
        elif opcode == 0x36:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_ABSF({', '.join(args)})")
        elif opcode == 0x37:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_SIGN({', '.join(args)})")
        elif opcode == 0x38:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_SIGNF({', '.join(args)})")
        elif opcode == 0x39:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_MIN({', '.join(args)})")
        elif opcode == 0x3A:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_MINF({', '.join(args)})")
        elif opcode == 0x3B:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_MAX({', '.join(args)})")
        elif opcode == 0x3C:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_MAXF({', '.join(args)})")
        elif opcode == 0x3D:
            args = [*map(self.var, argv)]
            self.write_line(f"EVT_CLAMP({', '.join(args)})")
        elif opcode == 0x3E:
            args = [*map(self.var, argv)]
