#! /usr/bin/env python3
"""Sprite sheet tables, shared by the build and by splat's sprite segment.

They live here rather than beside the segment so the build can read sprites
without splat installed.
"""
...
def unpack_color(data) -> Tuple[int, int, int, int]:
    """Widen an RGBA5551 colour, which the ROM stores big endian."""
    packed = int.from_bytes(data[0:2], byteorder="big")
    red, green, blue = ((packed >> shift) & 0x1F for shift in (11, 6, 1))
    widen = lambda channel: ceil(0xFF * (channel / 31))
    return widen(red), widen(green), widen(blue), (packed & 1) * 0xFF
